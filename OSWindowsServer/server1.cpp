...
class LexemeBuffer {
 ...
 public:
    explicit LexemeBuffer(std::vector<Lexeme> new_lexeme_list) {
        index = 0;
...
void get_server_port_from_argv(char*);
...
std::vector<Lexeme> string_to_lexemes(std::string expression_string) {
    ...
    while (lexemes[index].get_type() != LexemeType::NUMBER
        && lexemes[index].get_type() != LexemeType::RIGHT_BRACKET
        && index >= 0) {
...
double multiplication_or_division(LexemeBuffer* lexemes) {
  ...
  // Умножаем текущий результат умножения/деления
  // на следующую минимальную составную часть
                current_value =
                    current_value
                    * number_or_expression_in_brackets(lexemes);
                break;
            case LexemeType::DIVIDE:
  // Делим текущий результат умножения/деления
  // на следующую минимальную составную часть
                current_value =
                    current_value
                    / number_or_expression_in_brackets(lexemes);
...
std::string figure_expression(char* expression_string) {
    ...
    LexemeBuffer *lexemes =
        new LexemeBuffer(string_to_lexemes(expression_string));
    std::string result = std::to_string(expression(lexemes));
    delete lexemes;
...
DWORD WINAPI client_handler(LPVOID connection_number) {
    ...
    while (true) {
        ...
        recv(
            connections[(intptr_t)connection_number],
            reinterpret_cast<char*>(&expression_size),
            sizeof(int),
            0);
        /*
        thread_output("Клиент " +
        std::to_string((int)connection_number) +
            ": хочет передать " +
            std::to_string(expression_size)+" символов.");
        */
        ...
        if (is_string_equal(expression, "End of operation")) {
            thread_output("Клиент " +
                std::to_string((intptr_t)connection_number) +
                ": закончил сеанс.");
            delete[] expression;
            break;
        } else {
            thread_output("Клиент " +
                std::to_string((intptr_t)connection_number) +
                ": выражение: " + std::string(expression));
        }
        /*
        for (int i = 0; i < expression_size; i++) {
            thread_output("Клиент " +
                std::to_string((int)connection_number) +
                ": выражение[" + std::to_string(i) + "]: " + expression[i]);
        }
        */
        for (int i = 0; i < expression_size; i++) {
            if ((static_cast<int>(expression[i]) <
                static_cast<int>('0')
                || static_cast<int>(expression[i]) >
                static_cast<int>('9'))
            ...
            if (i > 0) {
                ...
                if (expression[i - 1] == ')') {
                    if (expression[i] != '+' &&
                        ...
                        snprintf(
                            result,
                            sizeof("Uncorrect expression"),
                            "Uncorrect expression");
                        is_expression_correct = false;
                        break;
                    }
                }
                if (static_cast<int>(expression[i-1]) >=
                    static_cast<int>('0')
                    && static_cast<int>(expression[i-1]) <=
                    static_cast<int>('9')) {
                    if (expression[i] == '(') {
                        snprintf(
                            result,
        ...
        if (is_expression_correct) {
            snprintf(
                result,
                sizeof("Uncorrect expression"),
                figure_expression(expression).c_str());
        }
        send(connections[(intptr_t)connection_number], result, RESULT_SIZE, 0);
        thread_output("Клиент " +
            std::to_string((intptr_t)connection_number) +
            ": результат: " + std::string(result));
        delete [] expression;
    }
    available_connections.push((intptr_t)connection_number);
    return 0;
}
...
void create_new_thread(intptr_t thread_name) {
    ...
    threads[thread_name] =
        CreateThread(
                    ...
                    0,
                    client_handler,
                    reinterpret_cast<void*>(thread_name),
                    0,
                    &new_thread_id);
...
bool create_server_socket(SOCKET* created_socket) {
    ...
    // ориентированный на соединений потоков байтов или пакетов
    ...
    // с их помощью программа может формировать
    // собственные заголовки в сообщении.
    ...
    // с этого момента данный сокет (его имя)
    // будет ассоциироваться с конкретным процессом,
    // который «висит» по указанному адресу и порту.
    // Объект структуры инкапсулирующий информацию о семействе адресов
    ...
    */
    // Локальный IP с которым будет связан сокет
    server_address.sin_addr.s_addr = inet_addr(SERVER_IP);
    // Номер порта, с которым будет связан сокет
    server_address.sin_port = htons(SERVER_PORT);

    if (bind(
        (*created_socket),  // Созданный сокет
        // Адрес к которому он привязывается
    ...
    // Если больше клиентов попробует подключится, они будут сброшены
    int max_connected_process = total_connections;
    if (listen(
        (*created_socket),
        SOMAXCONN_HINT(max_connected_process))
        == SOCKET_ERROR) {
...
void listen_server_connection(const SOCKET& listening_socket) {
    // Accept
    // Подтверждение подключения (обычно на стороне сервера)
    ...
    while (1) {
        ...
        while (AcceptSocket == SOCKET_ERROR) {
            /*
            accept("слушающий" сокет на стороне Сервера,
            sockaddr*  указатель на пустую структуру sockaddr 
                в которую будет записана информация по подключившемуся Клиенту,
            sizeof(sockaddr*) )*/
            AcceptSocket = accept(listening_socket, NULL, NULL);
        }
...
void get_server_port_from_argv(char* argv) {
    SERVER_PORT = 0;
    int i = 0;
    while (argv[i] != NULL) {
        if (static_cast<int>(argv[i]) < static_cast<int>('0')
            || static_cast<int>(argv[i]) > static_cast <int>('9')) {
            SERVER_PORT = 10;
            break;
        }
        SERVER_PORT =
            (SERVER_PORT +
                static_cast<int>(argv[i]) - static_cast<int>('0'))
            * 10;
        i++;
    }
...
int main(int args, char*argv[]) {
    total_connections = 2;
    connections = new SOCKET[total_connections];
    threads = new HANDLE[total_connections];
    if (args > 1) {
        get_server_port_from_argv(argv[1]);
    } else {
        SERVER_PORT = 1;
    }
