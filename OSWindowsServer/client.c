...
int socket_connect(){
    ...
    if (connect(connect_socket, (struct sockaddr *)&connect_address, sizeof(connect_address)) < 0) {
        perror("connect");
        getchar();
        return 0;
    }
...
char* generate_expression(int*string_size){
    ...
    *string_size=0;
    ...
    while(min_lexemes>0){
        ...
        //printf(" %s\n", string);            
        if(string[(*string_size)-1]>='0'&&string[(*string_size)-1]<='9'){
        dice=rand()%100;
        }else{
            dice=rand()%25; 
                ...
                if(dice<=55){
                    ...
                    if(dice<=70){
                        ...
                        if(dice<=85){
                            ...
                            if(dice<=95){
                                if(string[(*string_size)-1]!='+'&&string[(*string_size)-1]!='-'&&string[(*string_size)-1]!='*'&&string[(*string_size)-1]!='/'){                                   
                                    switch(rand()%4){
                                        case 0:
                                            string[(*string_size)++]='+'; 
                                            break;
